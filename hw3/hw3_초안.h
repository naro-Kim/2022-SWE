...
public:
	...
	void enterDeleteInfo(Account* account);
...
class SearchProduct {
	...
	vector<Seller*> listSeller; // 판매자 객체 포인터 벡터
	Seller *searchProductSeller; // 검색한 상품의 판매자를 가리키는 포인터
	Product *searchProduct; //검색한 상품을 가리키는 포인터
public:
	SearchProduct(vector<Seller*> sellerList);
	void showSearchProductInfo(string productName);
	Seller* getSearchProductSeller(); // 검색 결과 상품 판매자 전달
	Product* getSearchProduct(); // 검색 결과 상품 전달
};
...
public:
	...
	int findProductOrder(string name);
